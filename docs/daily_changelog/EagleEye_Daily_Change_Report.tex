...
\date{Report revised: 19 May 2026 \\ \small Synced through the 96$\times$96 RGB MobileNetV1 0.2 sketchboard deployment and AI-assisted hard-negative capture update}

\begin{document}
\maketitle

\begin{abstract}
This document records project evolution in a professional, date-ordered format. Entries follow \texttt{git log} on \texttt{main}. The May 2026 additions now cover the model-training layout, hard-negative collection, knowledge distillation experiments, Edge Impulse grayscale/RGB testing, and the firmware/runtime lessons learned while moving from lab training to ESP32-CAM validation.
\end{abstract}
...
\subsection{16 May 2026 (\texttt{60b59617})}
...
\subsection{17 May 2026 (\texttt{a07039ba})}
\textbf{Hard-negative capture expansion and Edge Impulse deployment test.} This day focused on collecting a larger real-camera dataset and checking which model export path was actually usable on the ESP32-CAM. New human and non-human captures were added under \texttt{sketchboard/dataset/Humans/} and \texttt{sketchboard/dataset/NonHuman/}, while older tracked capture files from 9--13 May were cleaned out where they had been superseded by the newer organized run.

\textbf{Changed files.} Important files include \texttt{sketchboard/dataset/*20260517*.jpg}, \texttt{sketchboard/firmware/hard\_negative\_capturer/hard\_negative\_capturer.ino}, \texttt{collect\_hard\_negatives.py}, \texttt{simple\_server.py}, \texttt{firmware/tools/hard\_negative\_capturer/}, \texttt{models/MODEL\_VERSIONS.md}, \texttt{models/tflite\_to\_cpp\_header.py}, and archived model headers such as \texttt{model\_v1.0\_baseline.h}, \texttt{model\_v6.0\_edge\_impulse\_final.h}, and \texttt{model\_v6.1\_edge\_impulse\_grayscale.h}.

\textbf{New method.} The comparison changed from ``does the model classify this image?'' to ``which deployment path behaves correctly on hardware?'' The older Edge Impulse RGB export expected \texttt{48x48x3} input, while the newer grayscale export used \texttt{48x48x1}, matching the firmware pipeline. The sketchboard firmware was simplified into a serial-only detector so testing could focus on clear \texttt{Human detected} / \texttt{Not detected} output and timing logs instead of the earlier Wi-Fi/web UI flow.

\textbf{Firmware/runtime lesson.} The ESP32 tests also exposed a runtime cost: broad \texttt{AllOpsResolver} usage can make inference unnecessarily heavy. The hard-negative capturer path therefore moved toward \texttt{MicroMutableOpResolver}, registering only the operations required by the selected model. The Flask capture server was made easier to debug with a \texttt{/test} endpoint, GET/POST handling on \texttt{/save}, and request-size logging so upload problems are visible during collection.

\textbf{Documentation link.} \texttt{README.md}, \texttt{PROJECT\_MANUAL\_START.md}, and \texttt{models/MODEL\_VERSIONS.md} were updated around this workflow so the report, setup guide, and model archive describe the same RGB-vs-grayscale decision and the same hard-negative capture process.

\subsection{19 May 2026}
\textbf{RGB MobileNetV1 0.2 model selection and ESP32-CAM runtime debugging.} The project moved from the earlier $48{\times}48$ greyscale custom-tiny path toward a stronger $96{\times}96$ RGB transfer-learning pipeline for sketchboard experiments. The motivation was practical: the $\sim$5k-parameter custom CNN delivered low latency, but failed to generalize reliably in real camera scenes because the $48{\times}48$ greyscale crop removed too much human detail. The new candidate uses a pretrained MobileNetV1 backbone with width multiplier 0.2, a 32-neuron head, 0.25 dropout, class weighting, full image augmentation, 40 epochs, learning rate 0.00035, and batch size 16.

\textbf{Validation result.} The selected INT8 deployment reached 89.86\% validation accuracy with a confusion matrix of \texttt{[[59, 12], [3, 74]]} in label order \texttt{human}, \texttt{nonhuman}. This corresponds to approximately 95.2\% human precision, 83.1\% human recall, and 88.1\% human F1. The result documents the core TinyML trade-off for EagleEye: the smallest possible model is not necessarily the best model; the deployed model must balance accuracy, latency, memory, and real-world reliability on the ESP32-CAM S1.

\textbf{Firmware/runtime lesson.} Side-by-side PC and device checks showed that the camera/preprocessing path and the inference engine must be validated together. Earlier ESP32-CAM tests exposed saturated incorrect predictions when ESP-NN acceleration was enabled for the RGB MobileNet path, while the non-accelerated TFLite Micro path produced correct results. The generated Arduino library was therefore packaged with ESP-NN disabled for correctness on this target.

\textbf{Hard-negative capture update.} \texttt{sketchboard/firmware/hard\_negative\_capturer/hard\_negative\_capturer.ino} was converted from the old serial-only $48{\times}48$ greyscale TFLite sketch into a Wi-Fi AI-assisted hard-negative capturer using the installed \texttt{final\_inferencing} library. It now serves a browser UI, streams JPEG frames over WebSocket, exposes \texttt{/ai} for live human/nonhuman scores, and posts labelled JPEGs to the sketchboard Python receiver at \texttt{/save}. This restores the same workflow as the original hard-negative capturer while testing the new $96{\times}96$ RGB MobileNetV1 model.

\textbf{New/updated tooling.} Added a repeatable training script, \texttt{tools/edge\_impulse/retrain\_rgb\_96\_mobilenetv1\_a2\_smart\_no\_espnn.py}, and kept diagnostic helpers under \texttt{tools/diag/} for ESP32 frame dumps and PC/device inference comparison. \texttt{models/MODEL\_VERSIONS.md} was updated to record the architecture, metrics, and deployment trade-offs for the new sketchboard model.

\subsection{7--9 June 2026}
\textbf{App-controlled pan head (two-board servo) and smoother live preview.} This session added remote camera-pan control driven from the mobile app, and fixed the live-stream stutter and brownout resets observed while panning the camera.

\textbf{Servo pan path (app $\rightarrow$ main $\rightarrow$ helper).} A second ESP32-CAM acts as a dedicated \emph{helper} (camera disabled) that drives one servo as a pan head. The mobile app sends an absolute angle to the main board over HTTP (\texttt{GET /servo?angle=N} on port~80, handled in \texttt{camera\_web\_server.h}); the main forwards it to the helper over a one-wire UART link (main \texttt{GPIO15}~TX $\rightarrow$ helper \texttt{GPIO13}~RX, common ground) as \texttt{ANGLE:N}. The endpoint runs in the HTTP server task, so panning works even while a live preview is open. The legacy auto-detect path still notifies the helper (\texttt{HUMAN}) for a gentle alert sweep.

\textbf{Smooth, low-latency motion.} The helper firmware (\texttt{firmware/helper\_servo/}) uses a \textbf{non-blocking} stepper that eases the servo toward a target angle ($\sim$2\textdegree{} every 8~ms) instead of a blocking sweep; incoming commands only update the target, so a new command takes effect immediately and the UART is never stalled. On the app side (\texttt{LiveMonitorScreen.js}) a dependency-free slider (React Native \texttt{PanResponder}) maps touches by absolute screen position (\texttt{pageX} relative to the measured track) rather than child-relative \texttt{locationX}, fixing a low-angle mis-read/glitch; a latest-wins sender streams the angle live while dragging without piling up requests, plus $\pm$15\textdegree{} nudge, center, and preset buttons.

\textbf{Live-preview reliability.} The camera path was set to \texttt{CAMERA\_GRAB\_LATEST} with \texttt{CAMERA\_FB\_IN\_PSRAM} so the freshest frame is always served (no ``stuck'' frame when panning), and the WebSocket JPEG quality was lowered (40~$\rightarrow$~25) for smaller, more consistent frames over weak Wi-Fi. A brownout-detector disable (\texttt{RTC\_CNTL\_BROWN\_OUT\_REG}) was added as a band-aid against Wi-Fi current-spike resets during streaming, with the real fix noted as a proper 5~V supply and bulk capacitor.

\textbf{Supporting assets.} Added a standalone dual-ESP32 UART test rig under \texttt{firmware/tests/dual\_esp\_uart\_test/} (helper + main-stub + README documenting the boot-safe AI-Thinker pin set and wiring), and \texttt{tools/setup\_mosquitto\_for\_eagleeye.ps1} to make the local Mosquitto broker reachable by the camera on the PC hotspot (listener on all interfaces, anonymous access, firewall rule) for end-to-end testing.
