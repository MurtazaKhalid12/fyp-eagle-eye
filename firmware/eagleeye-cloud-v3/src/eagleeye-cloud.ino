 ...
 *
 *  Power mode: device sleeps in deep sleep between events.
 *    PIR on GPIO2 fires ext0 wakeup → full boot → WiFi/MQTT/AI.
 *    After SLEEP_INACTIVITY_MS of no PIR/detection/MQTT, device sleeps again.
 *
...
// --- activity tracking for deep sleep ---
...
void run_ai_step() {
  ...
  if (detected) {
    clear_scene_count = 0;
    g_last_activity_ms = millis();                   // reset sleep timer on detection
...
// ---------------------------------------------------------------
//  Deep sleep entry — publish offline, disconnect, show OLED, sleep.
//  PIR (GPIO2 HIGH) triggers ext0 wakeup on next motion event.
// ---------------------------------------------------------------
void enter_deep_sleep() {
  Serial.printf("[SLEEP] No activity for %lu s — deep sleep. PIR will wake.\n",
                SLEEP_INACTIVITY_MS / 1000);

  g_rtc_armed = is_system_armed;            // save armed state to RTC RAM

  if (client.connected()) {
    // Publish offline LWT so the app knows the device is sleeping
    const char *offlineMsg = "{\"online\":false,\"sleeping\":true}";
    client.publish(topic_status().c_str(), (const uint8_t *)offlineMsg, strlen(offlineMsg), true);
    delay(80);
    client.disconnect();
  }
  ...
  pir_configure_wakeup();    // ext0: GPIO2 HIGH wakes the device
  esp_deep_sleep_start();    // CPU stops here; resumes at setup() on wakeup
}
...
void setup() {
  ...
  // Show wakeup reason on the event line
  {
    char bootMsg[22];
    if (from_pir)
      snprintf(bootMsg, sizeof(bootMsg), "PIR Wake #%u", (unsigned)g_boot_count);
    else
  ...
  g_last_activity_ms = millis();                       // start the inactivity clock after full init
...
void loop() {
  ...
  // PIR check — reset sleep timer and log on rising edge
  if (is_system_armed && pir_motion_detected()) {
  ...
  // --- deep sleep check (only in idle AI mode) ---
  // Use the latest of: PIR activity, AI detection, MQTT command
  {
    unsigned long lastActive = (g_last_activity_ms > g_last_cmd_ms) ? g_last_activity_ms : g_last_cmd_ms;
    if (millis() - lastActive > SLEEP_INACTIVITY_MS) {
      enter_deep_sleep();
    }
  }
