...
#include "eagleeye_oled.h"        // oled_set_streaming() — show LIVE screen, freeze redraws
...
inline void relay_start() {
  ...
  oled_set_streaming(true);       // instant static "LIVE" screen; no more OLED redraws till stop
...
inline void relay_stop() {
  ...
  g_relay_connected = false;
  g_mode = MODE_AI;
  oled_set_streaming(false);      // back to the live status screen
}
