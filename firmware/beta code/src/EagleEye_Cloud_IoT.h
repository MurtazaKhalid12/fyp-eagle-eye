...
int    g_req_servo_angle = -1;       // pan  target (-1 = none pending)  GPIO12
...
inline void publish_status() {
  ...
  oled_set_rssi(rssi);                       // update OLED signal bars
  EE_JSON(d, 256);
  d["online"] = true;
...
inline void mqtt_callback(char *topic, byte *payload, unsigned int length) {
  ...
  if (!strcmp(type, "arm")) {
    ...
    oled_log(is_system_armed ? "CMD: System Armed" : "CMD: Disarmed");
    publish_status();
    ...
    char buf[32];
    snprintf(buf, sizeof(buf), "CMD: Pan=%d T=%d", pan, tilt);
    oled_log(buf);
...
inline void init_wifi_mqtt() {
  ...
  oled_set_wifi("Connecting...");
  oled_log("WiFi Connecting");
  WiFi.mode(WIFI_STA);
  ...
  if (WiFi.status() == WL_CONNECTED) {
    oled_set_wifi("Connected");
    oled_log("WiFi OK!");
  } else {
    oled_set_wifi("Timeout");
    oled_log("WiFi Timeout");
  }
...
inline void mqtt_service() {
  ...
  oled_set_mqtt("Connecting...");
  oled_log("MQTT Connecting");
  if (client.connect(cid.c_str(), g_cfg.mqttUser.c_str(), g_cfg.mqttPass.c_str(),
                     willTopic.c_str(), 1, true, willMsg)) {     // LWT: offline on drop
    Serial.println("ok");
    oled_set_mqtt("Connected");
    oled_log("MQTT OK!");
    client.subscribe(topic_cmd().c_str(), 1);
    publish_status();
  } else {
    oled_set_mqtt("Failed");
    oled_log("MQTT Failed");
    // Diagnostic: wifi=3 means WL_CONNECTED (joined hotspot OK). If wifi=3 + a valid
    // 192.168.137.x ip but still rc=-2, the TLS socket failed (uplink/MTU), not Wi-Fi.
    Serial.printf("failed rc=%d  wifi=%d  ip=%s  rssi=%d  heap=%u (retry 5s)\n",
