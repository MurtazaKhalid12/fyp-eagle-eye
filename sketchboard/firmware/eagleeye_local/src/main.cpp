 ...
 *  ESP32-CAM AI Thinker · 96x96 GRAYSCALE human / nonhuman detector
 *  (also trained to classify an all-black frame as non-human)
 *
 ...
 *  (src/model_data.h, g_model[]) is 100% OURS — trained offline by
 *  sketchboard/train_ei_local.py, a faithful local replica of the Edge
 *  Impulse "final" project. Edge Impulse is used ONLY as the int8 runtime,
 *  because it bundles ESP-NN (hardware-accelerated int8 kernels). ESP-NN is
 *  enabled in the engine (EI_CLASSIFIER_TFLITE_ENABLE_ESP_NN = 1).
 *
 ...
 *
 *  Input convention (MUST match training): RGB565 -> luma gray =
 *  (77R+150G+29B)>>8 (1 channel) -> normalise /255 -> quantise with the
 *  model's own input scale/zero-point (1/255, -128).
...
// Centre-square crop of the QVGA frame, nearest-neighbour resize to 96x96, and
// convert to 1-channel GRAYSCALE using the SAME luma as training:
// gray = (77*R + 150*G + 29*B) >> 8. Geometrically identical to training.
static void rgb565_to_gray_resize_crop(const uint8_t* src, int src_w, int src_h,
    ...
    for (int y = 0; y < dst_h; y++) {
        for (int x = 0; x < dst_w; x++) {
            ...
            g = (g << 2) | (g >> 4);
            b = (b << 3) | (b >> 2);
            // 1 byte/pixel luminance — identical weights to train_local_gray.py
...
// Classify one RGB565 camera frame (grayscale -> int8 -> model) and publish the
// result into the globals above. Called per streamed frame by the web server.
void infer_on_frame(camera_fb_t* fb) {
    static uint8_t gray[IMG_W * IMG_H];   // 1 channel (grayscale)
    rgb565_to_gray_resize_crop(fb->buf, fb->width, fb->height, gray, IMG_W, IMG_H);
    ...
    for (int i = 0; i < n; i++) {
        float norm = (float)gray[i] / 255.0f;
