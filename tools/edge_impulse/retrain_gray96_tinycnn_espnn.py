...
ZIP_PATH = OUT_DIR / "ei_arduino_library_gray96_tinycnn_espnn.zip"
UNPACK_DIR = OUT_DIR / "ei_arduino_library_gray96_tinycnn_espnn"
...
VISUAL_LAYERS = [
    ...
    {"type": "dropout", "dropoutRate": 0.25, "enabled": True},
]
TRAINING_CYCLES = 60      # from-scratch needs more epochs than transfer
...
def main() -> None:
    ...
    print("Step 2) Set impulse: 96x96 + custom Keras (tiny CNN) block...", flush=True)
    rest_post("impulse", {
        "name": "EagleEye 96x96 Grayscale Tiny CNN human",
        "inputBlocks": [{
    ...
    print("Step 5) Generate 96x96 grayscale features...", flush=True)
    ...
    print("=" * 64, flush=True)
    print(" Tiny CNN (greyscale 96x96) Arduino library — ESP-NN ENABLED", flush=True)
