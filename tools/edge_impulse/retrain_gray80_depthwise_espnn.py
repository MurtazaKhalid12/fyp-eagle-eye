...
ZIP_PATH = OUT_DIR / "ei_arduino_library_gray80_depthwise_espnn.zip"
UNPACK_DIR = OUT_DIR / "ei_arduino_library_gray80_depthwise_espnn"
...
def main() -> None:
    ...
    print("Step 2) Set impulse: 80x80 + custom Keras block...", flush=True)
    rest_post("impulse", {
        "name": "EagleEye 80x80 Grayscale Depthwise CNN human",
    ...
    print("Step 7) Build Arduino library (ESP-NN ENABLED)...", flush=True)
    ...
    print("=" * 64, flush=True)
    print(" Depthwise-separable tiny CNN (greyscale 80x80) — ESP-NN ENABLED", flush=True)
